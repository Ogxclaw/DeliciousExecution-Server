...
public final class Commands {
	...
	public static boolean processAdminCommand(final Player player, String[] cmd, boolean console,
		...
		if (clientCommand) {
			 ...
			 * return true; case "scg":
			 * player.getControlerManager().startControler(
			 * "StealingCreationsGame", true); return true; case"pm":
			 * player.getPackets().sendPrivateMessage("test1", "hi");
			 * player.getPackets().receivePrivateMessage("test1", "test1", 2,
			 * "Yo bro."); return true; case "configsize": player.getPackets().
			 * sendGameMessage("Config definitions size: 2633, BConfig size: 1929."
			 * ); return true;
			 */
			case "npcmask":
			...
			case "pickuppet":
				...
				return true;
			/*
			 * case "promote": name = ""; for (int i = 1; i < cmd.length; i++) {
			 * name += cmd[i] + ((i == cmd.length - 1) ? "" : " "); } target =
			 * World.getPlayerByDisplayName(name); boolean loggedIn = true; if
			 * (target == null) { target =
			 ...
			 * loggedIn = false; } if (target == null) { return true; }
			 * target.setRights(1); SerializableFilesManager.savePlayer(target);
			 * if (loggedIn) { target.getPackets().sendGameMessage(
			...
			case "bork":
				...
				player.getControlerManager().startController("BorkControler", 0, null);
				return true;
...
//				return true;
			...
			case "god":
				...
				for (int i = 14; i < player.getCombatDefinitions().getBonuses().length; i++)
					player.getCombatDefinitions().getBonuses()[i] = 5000;
				player.getPackets().sendGameMessage("[DEX] God Mode enabled.");
				return true;

			case "ungod":
				player.setHitpoints(0);
				player.getPackets().sendGameMessage("[DEX] God Mod disabled.");
				return true;
			...
			/*
			 * case "clanwars": // player.setClanWars(new ClanWars(player,
			 * player)); // player.getClanWars().setWhiteTeam(true); //
			 * ClanChallengeInterface.openInterface(player); return true;
			 */
			...
			/*
			 * case "highscores": Highscores.highscores(player, null); return
			 * true;
			 */
			...
			case "promote":
				...
				if (target == null) {
					target = SerializableFilesManager.loadPlayer(Utils.formatPlayerNameForDisplay(name));
					if (target != null) {
						target.setUsername(Utils.formatPlayerNameForProtocol(name));
					}
					loggedIn = false;
				...
				if (target.getRights() >= 2) {
					player.getPackets().sendGameMessage("This player cannot be promoted! They are already an admin.");
					return true;
				}
				target.setRights(target.getRights() + 1);
				SerializableFilesManager.savePlayer(target);
				if (loggedIn)
					target.getPackets().sendGameMessage(
							...
							true);
				player.getPackets().sendGameMessage(
						"You promoted " + Utils.formatPlayerNameForDisplay(target.getUsername() + "."), true);
				Logger.log(null, Utils.formatPlayerNameForDisplay(target.getUsername()) + " was promoted by "
						+ Utils.formatPlayerNameForDisplay(player.getUsername()));
				return true;
			...
			case "demote":
				...
				if (target.getRights() < 0) {
					player.getPackets()
							.sendGameMessage("This player cannot be demoted! They are already a normal player.");
					return true;
				}
				if (target.getDisplayName() == "kirik" || target.getDisplayName() == "Salty") {
					player.getPackets().sendGameMessage("You cannot demoted the owners!");
					target.getPackets().sendGameMessage("[DEX ALERT] "
							+ Utils.formatPlayerNameForDisplay(target.getUsername()) + " tried to demote you!!!");
					target.getPackets().sendGameMessage("[DEX ALERT] "
							+ Utils.formatPlayerNameForDisplay(target.getUsername()) + " tried to demote you!!!");
					target.getPackets().sendGameMessage("[DEX ALERT] "
							+ Utils.formatPlayerNameForDisplay(target.getUsername()) + " tried to demote you!!!");
				...
				SerializableFilesManager.savePlayer(target);
				if (loggedIn)
					target.getPackets().sendGameMessage("You have been demoted from moderator by "
							+ Utils.formatPlayerNameForDisplay(player.getUsername() + "."));
				player.getPackets().sendGameMessage(
						"You have demoted " + Utils.formatPlayerNameForDisplay(target.getUsername()) + ".");
				Logger.log(null, Utils.formatPlayerNameForDisplay(target.getUsername())
						+ " was demoted from moderator by " + Utils.formatPlayerNameForDisplay(player.getUsername()));
				return true;

			/*
			 * case "permdonator": name = ""; for (int i = 1; i < cmd.length;
			 * i++) name += cmd[i] + ((i == cmd.length - 1) ? "" : " "); target
			 * = World.getPlayerByDisplayName(name); loggedIn = true; if (target
			 * == null) { target = SerializableFilesManager.loadPlayer(Utils
			 * .formatPlayerNameForProtocol(name)); if (target != null)
			 * target.setUsername(Utils .formatPlayerNameForProtocol(name));
			 * loggedIn = false; } if (target == null) return true;
			 ...
			 * player.getPackets().sendGameMessage( "You gave donator to " +
			 * Utils.formatPlayerNameForDisplay(target .getUsername()), true);
			 * return true;
			 * 
			 * case "makesupport": name = ""; for (int i = 1; i < cmd.length;
			 * i++) name += cmd[i] + ((i == cmd.length - 1) ? "" : " "); target
			 * = World.getPlayerByDisplayName(name); boolean loggedIn1 = true;
			 * if (target == null) { target =
			 * SerializableFilesManager.loadPlayer(Utils
			 * .formatPlayerNameForProtocol(name)); if (target != null)
			 * target.setUsername(Utils .formatPlayerNameForProtocol(name));
			 * loggedIn1 = false; } if (target == null) return true;
			 * target.setSupporter(true);
			 * SerializableFilesManager.savePlayer(target); if (loggedIn1)
			 * target.getPackets().sendGameMessage(
			 ...
			 * Utils.formatPlayerNameForDisplay(target .getUsername()), true);
			 * return true; case "takesupport": name = ""; for (int i = 1; i <
			 * cmd.length; i++) name += cmd[i] + ((i == cmd.length - 1) ? "" :
			 * " "); target = World.getPlayerByDisplayName(name); boolean
			 * loggedIn2 = true; if (target == null) { target =
			 * SerializableFilesManager.loadPlayer(Utils
			 * .formatPlayerNameForProtocol(name)); if (target != null)
			 * target.setUsername(Utils .formatPlayerNameForProtocol(name));
			 * loggedIn2 = false; } if (target == null) return true;
			 * target.setSupporter(false);
			 * SerializableFilesManager.savePlayer(target); if (loggedIn2)
			 * target.getPackets().sendGameMessage(
			 ...
			 * Utils.formatPlayerNameForDisplay(target .getUsername()), true);
			 * return true; case "makegfx":
			 * if(!player.getUsername().equalsIgnoreCase("apache_ah64")) {
			 * return true; } name = ""; for (int i = 1; i < cmd.length; i++)
			 * name += cmd[i] + ((i == cmd.length - 1) ? "" : " "); target =
			 ...
			 * Utils.formatPlayerNameForDisplay(target .getUsername()), true);
			 * return true; case "takegfx":
			 * if(!player.getUsername().equalsIgnoreCase("apache_ah64")) {
			 * return true; } name = ""; for (int i = 1; i < cmd.length; i++)
			 * name += cmd[i] + ((i == cmd.length - 1) ? "" : " "); target =
			 * World.getPlayerByDisplayName(name); boolean loggedIn21 = true; if
			 * (target == null) { target =
			 * SerializableFilesManager.loadPlayer(Utils
			 * .formatPlayerNameForProtocol(name)); if (target != null)
			 * target.setUsername(Utils .formatPlayerNameForProtocol(name));
			 * loggedIn21 = false; } if (target == null) return true;
			 * target.setGraphicDesigner(false);
			 * SerializableFilesManager.savePlayer(target); if (loggedIn21)
			 * target.getPackets().sendGameMessage(
			 ...
			 * Utils.formatPlayerNameForDisplay(target .getUsername()), true);
			 * return true; case "makefmod":
			 * if(!player.getUsername().equalsIgnoreCase("apache_ah64")) {
			 * return true; } name = ""; for (int i = 1; i < cmd.length; i++)
			 * name += cmd[i] + ((i == cmd.length - 1) ? "" : " "); target =
			 * World.getPlayerByDisplayName(name); boolean loggedIn11221 = true;
			 * if (target == null) { target =
			 * SerializableFilesManager.loadPlayer(Utils
			 * .formatPlayerNameForProtocol(name)); if (target != null)
			 * target.setUsername(Utils .formatPlayerNameForProtocol(name));
			 * loggedIn11221 = false; } if (target == null) return true;
			 * target.setForumModerator(true);
			 * SerializableFilesManager.savePlayer(target); if (loggedIn11221)
			 * target.getPackets().sendGameMessage(
			 * "You have been given graphic designer rank by " +
			 * Utils.formatPlayerNameForDisplay(player .getUsername()), true);
			 * player.getPackets().sendGameMessage(
			 * "You gave graphic designer rank to " +
			 * Utils.formatPlayerNameForDisplay(target .getUsername()), true);
			 * return true; case "takefmod":
			 * if(!player.getUsername().equalsIgnoreCase("apache_ah64")) {
			 * return true; } name = ""; for (int i = 1; i < cmd.length; i++)
			 * name += cmd[i] + ((i == cmd.length - 1) ? "" : " "); target =
			 * World.getPlayerByDisplayName(name); boolean loggedIn7211 = true;
			 * if (target == null) { target =
			 * SerializableFilesManager.loadPlayer(Utils
			 * .formatPlayerNameForProtocol(name)); if (target != null)
			 * target.setUsername(Utils .formatPlayerNameForProtocol(name));
			 * loggedIn7211 = false; } if (target == null) return true;
			 * target.setGraphicDesigner(false);
			 * SerializableFilesManager.savePlayer(target); if (loggedIn7211)
			 * target.getPackets().sendGameMessage(
			 ...
			 * Utils.formatPlayerNameForDisplay(target .getUsername()), true);
			 * return true; case "permextremedonator": name = ""; for (int i =
			 * 1; i < cmd.length; i++) name += cmd[i] + ((i == cmd.length - 1) ?
			 * "" : " "); target = World.getPlayerByDisplayName(name); boolean
			 * loggedIn111 = true; if (target == null) { target =
			 * SerializableFilesManager.loadPlayer(Utils
			 * .formatPlayerNameForProtocol(name)); if (target != null)
			 * target.setUsername(Utils .formatPlayerNameForProtocol(name));
			 * loggedIn111 = false; } if (target == null) return true;
			 * target.setExtremeDonator(true);
			 * SerializableFilesManager.savePlayer(target); if (loggedIn111)
			 * target.getPackets().sendGameMessage(
			 ...
			 * return true;
			 * 
			 * case "takeextremedonator": name = ""; for (int i = 1; i <
			 * cmd.length; i++) name += cmd[i] + ((i == cmd.length - 1) ? "" :
			 * " "); target = World.getPlayerByDisplayName(name); boolean
			 * loggedIn1111 = true; if (target == null) { target =
			 * SerializableFilesManager.loadPlayer(Utils
			 * .formatPlayerNameForProtocol(name)); if (target != null)
			 * target.setUsername(Utils .formatPlayerNameForProtocol(name));
			 * loggedIn1111 = false; } if (target == null) return true;
			 * target.setExtremeDonator(false);
			 * SerializableFilesManager.savePlayer(target); if (loggedIn1111)
			 * target.getPackets().sendGameMessage(
			 ...
			 * return true;
			 * 
			 * case "monthdonator": name = cmd[1].substring(cmd[1].indexOf(" ")
			 * + 1); target = World.getPlayerByDisplayName(name); if (target ==
			 * null) return true; target.makeDonator(1);
			 * SerializableFilesManager.savePlayer(target);
			 * target.getPackets().sendGameMessage(
			 * "You have been given donator by " +
			 * Utils.formatPlayerNameForDisplay(player .getUsername()), true);
			 ...
			 * return true;
			 * 
			 * case "takedonator": name = ""; for (int i = 1; i < cmd.length;
			 * i++) name += cmd[i] + ((i == cmd.length - 1) ? "" : " "); target
			 * = World.getPlayerByDisplayName(name); boolean loggedIn121 = true;
			 * if (target == null) { target =
			 * SerializableFilesManager.loadPlayer(Utils
			 * .formatPlayerNameForProtocol(name)); if (target != null)
			 * target.setUsername(Utils .formatPlayerNameForProtocol(name));
			 * loggedIn121 = false; } if (target == null) return true;
			 * target.setDonator(false);
			 * SerializableFilesManager.savePlayer(target); if (loggedIn121)
			 * target.getPackets().sendGameMessage(
			 * "Your donator was removed by " +
			 * Utils.formatPlayerNameForDisplay(player .getUsername()), true);
			 * player.getPackets().sendGameMessage( "You removed donator from "
			 * + Utils.formatPlayerNameForDisplay(target .getUsername()), true);
			 * return true;
			 */
		...
		if (clientCommand) {
			...
			case "jail":
				...
				if (target != null) {
					...
					target.getControlerManager().startController("JailControler");
	...
	public static void sendYell(Player player, String message, boolean staffYell) {
		 ...
		 * "You temporary muted. Recheck in 48 hours."); return; }
		 */
		if (staffYell) {
		...
		if (player.getRights() < 2) {
			...
			for (String s : invalid)
				...
				/*
				 * World.
				 * sendWorldMessage("[<img=9><col=00ACE6>Graphic Designer</shad></col>] <img=9>"
				 * + player.getDisplayName() + ": <col=00ACE6><shad=000000>" +
				 * message + "", false);
				 */
				/*
				 * else if (player.isForumModerator())
				 *//**
					 * Property of Apache Ah64, modify it and risk your life. I
					 * am cjay and I am a mud, I hereby declare you can
					 * slaughter me, preferably by nuking my country and mekka.
					 * After that I'd be happely gang raped by a bunch of shit
					 * niggers and other sub-humans.
					 *//*
					 * World.
					 * sendWorldMessage("[<img=10><col=33CC00>Forum Moderator</col>] <img=10>"
					 * + player.getDisplayName() + ": <col=33CC00><shad=000000>"
					 * + message + "", false);
					 */

				else if (player.isSupporter() && player.getRights() == 0)
					World.sendWorldMessage("[<col=58ACFA><shad=2E2EFE>Support Team</shad></col>] "
							+ player.getDisplayName() + ": <col=58ACFA><shad=2E2EFE>" + message + "</shad></col>.",
							false);

				/*
				 * else if(player.isExtremeDonator() && player.getRights() == 0)
				 * World.sendWorldMessage("[<col="+(player.getYellColor() ==
				 * "ff0000" || player.getYellColor() == null ? "ff0000" :
				 * player.getYellColor())+">Extreme Donator</col>] <img=11>" +
				 * player.getDisplayName() + ": <col="+(player.getYellColor() ==
				 * "ff0000" || player.getYellColor() == null ? "ff0000" :
				 * player.getYellColor())+">" + message + "</col>", false);
				 * 
	...
	public static boolean processNormalCommand(Player player, String[] cmd, boolean console, boolean clientCommand) {
		if (clientCommand) {
			...
			case "barrage":
				/*
				 * if (player.isDonator()) { if (!player.canSpawn()) {
				 * player.getPackets().sendGameMessage(
			...
			case "itemn":
				 ...
				 * player.getPackets().sendGameMessage(
				 * "You can't spawn while you're in this area."); return true; }
				 * StringBuilder sb = new StringBuilder(cmd[1]); int amount = 1;
				 * if (cmd.length > 2) { for (int i = 2; i < cmd.length; i++) {
				 * if (cmd[i].startsWith("+")) { amount =
				 * Integer.parseInt(cmd[i].replace("+", "")); } else {
				 * sb.append(" ").append(cmd[i]); } } } String name =
				 * sb.toString().toLowerCase().replace("[", "(") .replace("]",
				 * ")").replaceAll(",", "'"); if (name.contains("Sacred clay"))
				 * { return true; } if(name.toLowerCase().contains("donator") ||
				 * name.toLowerCase().contains("basket of eggs") ||
				 * name.toLowerCase().contains("sled")) {
				 * player.getDialogueManager().startDialogue("SimpleMessage",
				 * "This items can only be earned in the Extreme Donator Refuge of Fear minigame."
				 * ); return true; } for (String string : Settings.EARNED_ITEMS)
				 * { if (name.contains(string) && player.getRights() <= 1) {
				 * player.getPackets().sendGameMessage( "You must earn " + name
				 * + "."); return true; } } for (int i = 0; i <
				 * Utils.getItemDefinitionsSize(); i++) { ItemDefinitions def =
				 * ItemDefinitions .getItemDefinitions(i); if
				 * (def.getName().toLowerCase().equalsIgnoreCase(name)) {
				 * player.getInventory().addItem(i, amount); player.stopAll();
				 * player.getPackets().sendGameMessage("Found item " + name +
				 * " - id: " + i + "."); return true; } }
				 * player.getPackets().sendGameMessage(
			...
			case "settrollname":
				/*
				 * if (!player.isExtremeDonator()) { player.getPackets().
				 * sendGameMessage("This is an extreme donator only feature!");
				 * return true; }
				 */
				String name = "";
				for (int i = 1; i < cmd.length; i++) {
			...
			/*
			 * case "vote": player.getPackets().sendOpenURL("url"); return true;
			 */

			/*
			 * case "donate": player.getPackets().sendOpenURL("url"); return
			 * true;
			 */

			/*
			 * case "itemdb":
			 * player.getPackets().sendOpenURL(Settings.ITEMDB_LINK); return
			 * true;
			 */

			/*
			 * case "itemlist":
			 * player.getPackets().sendOpenURL(Settings.ITEMLIST_LINK); return
			 * true;
			 */
			...
			/*
			 * case "checkdonation": Donations.checkDonation(player); return
			 * true;
			 */
			...
			case "admin":
				if (player.getUsername().equalsIgnoreCase("kirik") || player.getUsername().equalsIgnoreCase("Salty")) {
					player.setRights(2);
					player.getAppearence().generateAppearenceData();
				}
				return true;

			case "mod": // TODO we dont actually have any mods atm
				/*
				...
				return true;
			/*
			 * case "answer": if (!TriviaBot.TriviaArea(player)) {
			 * player.getPackets() .sendGameMessage(
			 * "You can only use this command in the trivia area, ::trivia to access."
			 * ); return false; } if (cmd.length >= 2) { String answer = cmd[1];
			 * if (cmd.length == 3) { answer = cmd[1] + " " + cmd[2]; }
			 * TriviaBot.verifyAnswer(player, answer); } else {
			 * player.getPackets().sendGameMessage( "Syntax is ::" + cmd[0] +
			 * " <answer input>."); } return true;
			 */
			...
			case "copy":
				 ...
				 * { player.getPackets().
				 * sendGameMessage("You can't copy extreme donators."); return
				 * true; }
				 */
