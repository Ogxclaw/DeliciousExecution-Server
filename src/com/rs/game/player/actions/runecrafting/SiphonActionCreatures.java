		...
		if (started) {
			...
			if (Utils.getRandom(level <= 50 ? 2 : 1) == 1) {
				...
				if(points >= 1) {
					...
					points -= pointsInt;
					Controller controler = player.getControlerManager().getController();
					if(controler instanceof RunespanController) 
						((RunespanController)controler).addInventoryPoints(pointsInt);
				}
